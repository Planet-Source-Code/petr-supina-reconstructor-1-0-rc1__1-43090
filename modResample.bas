...
Public cubic_a!, cubic_b!, cubic_c!, sinc_size&, sinc_window&

' Cardinal cubic spline function
' Depends on one variable: a (cubic_a in code)
...
Function windowed_sinc(ByVal X!) As Single
    ...
    ElseIf Abs(X) < sinc_size Then
        Select Case sinc_window
            Case wBartlett
                windowed_sinc = 1 - Abs(X) / sinc_size
            Case wBlackman
                windowed_sinc = pi * X / sinc_size
                windowed_sinc = 0.42! + 0.5! * Cos(windowed_sinc) + 0.08! * Cos(2 * windowed_sinc)
            Case wBlackmanHarris
                windowed_sinc = pi * X / sinc_size
                windowed_sinc = 0.42323! + 0.49755! * Cos(windowed_sinc) + 0.07922! * Cos(2 * windowed_sinc)
            Case wBohman
                windowed_sinc = X / sinc_size
                windowed_sinc = (1 - Abs(windowed_sinc)) * Cos(pi * windowed_sinc) + 1 / pi * Sin(pi * Abs(windowed_sinc))
            Case wCosine
                windowed_sinc = Cos(X * pi / (sinc_size * 2))
            Case wGauss
                windowed_sinc = 2 ^ (-X * X * 0.25!)
            Case wHamming
                windowed_sinc = 0.54! + 0.46! * Cos(pi * X / sinc_size)
            Case wHann
                windowed_sinc = 0.5! + 0.5! * Cos(pi * X / sinc_size)
            Case wLanczos
                windowed_sinc = pi * X / sinc_size
                windowed_sinc = Sin(windowed_sinc) / windowed_sinc
            Case wRectangular
                windowed_sinc = 1
            Case wWelch
                windowed_sinc = 1 - X * X / (sinc_size * sinc_size)
        End Select
        windowed_sinc = windowed_sinc * Sin(pi * X) / (pi * X)
    End If
End Function
...
Sub Resample(ByVal nType As eTypes, hBitmapDst As Picture, hBitmapSrc As Picture)
